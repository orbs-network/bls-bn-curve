...
contract dkg {
    ...
    enum Phase { Enrollment, Commit, PostCommit, End } // start from 0
        ...
        returns(bool)
    {
        ...
        for(uint16 i = 0; i < threshold; i++) {
            participants[senderIndex].publicCommitments[i][0] = pubCommit[2*i];
            participants[senderIndex].publicCommitments[i][1] = pubCommit[2*i+1];
        }
        ...
        if(committedNum == 0) {
            curPhase = Phase.End; // TODO: change to PostCommit
            emit PhaseChange(Phase.End);
        }
    ...
    function slash(uint16 slashedIndex) private {
        ...
        uint16 nParticipants = n;
        uint256 amount = this.balance/(nParticipants-1);

        for (uint16 i = 1; i < nParticipants; i++) {
            if (i != slashedIndex) {
